...
面向 Asteria Research Agent 的本地自动化测试工作台。项目把单元、接口、异步任务、性能验证和 Agent 行为评测放在同一套测试目录中，结果以运行批次、断言明细和失败证据的形式留存。

当前工作台基于原有 T2I Safety 控制台演进，Asteria 是本轮主测试对象；T2I 生成、图像裁判和数据质量能力作为兼容目标保留。科研 Agent 测试覆盖 Coordinator 输入契约、异步意图路由、只读 API、任务恢复、Agent Trace 与质量评测。真实研究请求和压测仍需要显式确认，默认使用 Mock，不会因为打开页面而产生费用。
...
- `backend/`：FastAPI API、测试目录、受控运行器和 T2I 兼容适配
- `tests/`：工作台自身的无模型契约测试
- `docs/`：产品、测试、部署和实现记录

## 安全与数据边界

项目不保存 API Key。服务端只从本机环境变量或显式配置文件读取密钥，前端只显示通道状态；测试运行器只接受注册的用例和目标，不接受任意 Shell 命令或任意 URL。Asteria API 冒烟只访问固定只读入口，不提交研究任务；个人实验输出、模型权重、`.env`、真实提示词数据和运行产物均不纳入仓库。
