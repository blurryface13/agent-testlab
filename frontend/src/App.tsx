  ...
  return <>
    ...
    {initialTab === "workbench" ? <>
      <section className="testing-toolbar panel">
        ...
        <label><span>执行工具</span><select value={tool} onChange={(event) => setTool(event.target.value)}>{catalog?.tools.filter((item) => item.executable !== false).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
...
function TestToolsPanel({ tools }: { tools: Array<{ id: string; name: string; kind: string; status: string; executable?: boolean; description: string; guide: string[]; asset?: string }> }) {
  return <section className="panel testing-record-panel"><div className="panel-heading"><div><p className="eyebrow">TOOLBOX</p><h2>测试工具说明</h2></div></div><p className="panel-description">同一测试目录按职责选择工具，工具状态清晰区分可执行、可导出、CI 主线、手动辅助与适配中。导出资产不包含密钥。</p><div className="tool-guide-list">{tools.map((tool) => <article className="tool-guide" key={tool.id}><div className="tool-guide-head"><strong>{tool.name}</strong><span className={`tool-state ${tool.status}`}>{tool.status === "ready" ? "已接入" : tool.status === "export" ? "可导出" : tool.status === "workflow" ? "CI 主线" : tool.status === "adapter" ? "适配中" : tool.status === "manual" ? "手动辅助" : "计划中"}</span></div><p>{tool.description}</p><ol>{tool.guide.map((step) => <li key={step}>{step}</li>)}</ol>{tool.asset && <a className="tool-asset" href={`/api/testing/assets/${tool.asset}`} download>下载无密钥示例 <span>↗</span></a>}</article>)}</div></section>;
}
